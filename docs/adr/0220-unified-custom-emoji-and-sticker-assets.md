...
2. **傳遞採 Model A（行內內嵌）為出貨型態**：訊息內容尾端附一段**分隔的資產清單** `nb-assets:v1:<JSON>`，JSON 為 `{ [shortcode]: { label, svg } }`（以短碼為鍵，直接對應文字內的 `:shortcode:`；儲存端另以 `contentHash(svg)` 作 id 去重），**只含本則文字實際引用的資產**；文字內以 `:shortcode:` 引用，**解析順序＝本則清單（依 shortcode）→ 本機庫 → 保留原字**（未知則顯示字面，優雅退化）。emoji 匯入時**降尺寸至 ≤64px**，使多顆仍安全落在 NIP-44 明文上限（65535）內，並設**每則資產預算**（清單總量 ≤ 48 KiB，超出則拒送並提示）。**Model B（內容定址省頻寬）與 C（org 共用集）列為後續 ADR。**
...
- **負面／已知殘餘風險**：
  ...
  - **庫變每身分獨立（實作發現）**：桌面 App 的 JS 端無加密金鑰（nsec 在 OS 金鑰庫、加密全在 Rust 側），故庫**存進 AppStorage**（Tauri＝Rust META part AES-256-GCM／瀏覽器＝dek）才能兩模式都加密；AppStorage 依 pubkey 命名空間 → 庫由「全域」變「**每身分獨立**」（多數單一身分無感、多身分更符合隱私）；一次性遷移把舊全域明文庫複製進各身分的加密庫（每身分一次，flag 防重跑）。
- **實作結果（全 6 步落地）**：core `custom-assets`（清單/短碼/LRU）、統一 `CustomAsset` 庫、送出內嵌＋行內渲染、收到自動收藏、挑選器 emoji 分頁（匯入＋短碼）、`:` 自動補全、設定開關、加密 AppStorage 落地＋遷移。core 352＋engine 278＋desktop 464 綠。
- **後續行動／待辦**：
