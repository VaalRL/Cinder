...
**採選項 1：按收件人 pubkey 前綴分片。** 本 ADR 定方向；shard 數、遷移、presence 連線策略為開放子問題（見下）。
...
- **presence 看聯絡人 → 連該聯絡人的片。** 但這條由 ADR-0240（真隱身，完全不訂閱）與 P2P 卸載
  大幅減輕（見「理由」的連線數分析）。
...
- **血條收益是無條件的。** 分片後一片崩＝**只影響 `shard(該片)` 的 ~1/N 使用者的收發**，他們重連到
  ...
  降為「一崩 1/N」。
- **🔴 擴充/連線數收益是**有條件的**（誠實面）。** 若 presence 仍走跨分片心跳訂閱：一個聯絡人分佈廣的
  client 要連 ~所有片（自己片＋每個聯絡人所在片）→ **每 client 連線數 ×N，總連線數不降、只分散**，
  每片的連線數 ≈ 全體 client 數 → **與單一全域 DO 相同**！所以擴充收益**取決於降低 relay presence
  依賴**：靠 **P2P 卸載**（正在對話者）＋ **真隱身（ADR-0240，完全不訂閱）** ＋「presence 是 per-對話
  而非全清單被動訂閱」的觀念轉變。只發訊給少數人的 client，只需連自己片＋少數活躍對話片 → 連線少、
  分片才真正省。
...
1. **shard 數。** 16（連線 ≤16、血條 1/16、粗）vs 256（連線可達 256、血條 1/256、細）。血條優先傾向
   先取**小數**（16），擴充需要時再切細。
2. **遷移。** 單一全域 DO 的 SQLite 已存**全體離線留言**；分片後每片各自存。需 dual-write 過渡或 cutover
   ——這是本 ADR 最大的**操作風險**。錨點稀少、離線留言 7 天過期，或可「切換後讓舊留言自然過期」而非搬遷。
3. **presence 跨分片連線爆炸。** 是否設**專用 presence 層/片**，或接受並靠 P2P／真隱身把 relay presence
   降到少數活躍對話。與 ADR-0237 的量測校正（Tier 0 覆蓋有限）一起看。

## 後果

- 正面：
  - **血條 1/1 → 1/N（無條件）**：C1 那類「一崩全崩」結構性消除；一片崩只影響其 1/N 使用者的收發。
  - **擴充天花板移除（有條件）**：搭配降低 relay presence，單片負載可控。
  - **重用既有 outbox**（ADR-0034），分片由 pubkey 可算、免 hint；非重寫。
  - 使 ADR-0237 Tier 2（多連線）更可行（負載分散）。
- 負面 / 已知殘餘風險：
  - **presence 跨分片連線**：不降低 relay presence 依賴時，連線數收益被抵銷（只分散不減少）。
  - **遷移操作風險**：全域 DO 的離線留言搬遷/切換。
  - **client 連線數上升**（自己片＋活躍對話片＋聯絡人片），對行動端電量/連線管理是成本。
- 後續行動 / 待辦：
  1. 決定 shard 數（建議先 16，血條優先）與遷移策略（傾向「切換＋舊留言自然過期」避免搬遷）。
  2. 決定 presence 跨分片策略（專用層 vs 靠 P2P/真隱身降依賴）——與 ADR-0237 Tier 0 校正一起。
  3. 與 ADR-0237 Tier 2（臨時身分連線）綁定，算連線總帳後再定是否一起做。
  4. prototype：`worker.ts` 依 URL 路徑選片；客戶端 `shard(pubkey)` 與 outbox 路由；血條測試（一片崩不影響他片）。
